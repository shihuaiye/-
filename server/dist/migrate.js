  ...
  `);
    ...
    try {
        ...
        for (const u of users) {
            await pool.query(`INSERT IGNORE INTO users (id, username, password, role, status, reviewNote, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                u.id,
                u.username,
                u.password,
    ...
    try {
        ...
        for (const p of products) {
            await pool.query(`INSERT IGNORE INTO products (id, title, description, price, category, images, campus, brand, model, memory, latitude, longitude, status, rejectionReason, sellerId, sellerName, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                p.id,
                p.title,
                p.description,
    ...
    try {
        ...
        for (const m of messages) {
            await pool.query(`INSERT IGNORE INTO messages (id, productId, fromUserId, fromUsername, toUserId, toUsername, content, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                m.id,
                m.productId || "",
                m.fromUserId,
                m.fromUsername,
