...
router.get("/recommendations", auth, async (req, res) => {
    ...
    let recommended = approvedProducts.filter((p) => purchasedCategories.has(p.category) && !userOrders.some((up) => up.productId === p.id));
    if (recommended.length < 3) {
        const remaining = approvedProducts.filter((p) => !recommended.includes(p) && !userOrders.some((up) => up.productId === p.id));
