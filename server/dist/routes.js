...
import { DATA_DIR, initDb, newId, readMessages, readProducts, readUsers, writeMessages, writeProducts, writeUsers, } from "./db.js";
...
router.post("/auth/register", async (req, res) => {
    ...
    const user = {
        ...
        role: body.role,
        status: isAdmin ? "pending_review" : "active",
...
router.get("/conversations", auth, async (req, res) => {
    const messages = await readMessages();
    const userMessages = messages.filter((m) => m.fromUserId === req.userId || m.toUserId === req.userId);
    ...
    const conversations = Array.from(conversationMap.values()).sort((a, b) => b.lastTime.localeCompare(a.lastTime));
...
router.get("/conversations/:userId", auth, async (req, res) => {
    ...
    const chatMessages = messages
        .filter((m) => (m.fromUserId === req.userId && m.toUserId === targetUserId) ||
...
router.put("/admin/users/:id", auth, adminOnly, async (req, res) => {
    ...
    if (body.reviewNote !== undefined)
        target.reviewNote = body.reviewNote;
...
router.post("/products/:id/sold", auth, async (req, res) => {
    const products = await readProducts();
    const idx = products.findIndex((p) => p.id === req.params.id);
    if (idx === -1)
        return res.status(404).json({ success: false, message: "商品不存在" });
    ...
    const ordersData = JSON.parse(await fs.readFile(path.join(DATA_DIR, "orders.json"), "utf-8").catch(() => "[]"));
    const newOrder = {
        id: newId("o"),
...
router.get("/orders", auth, async (req, res) => {
    ...
    const userOrders = ordersData.filter((o) => o.buyerId === req.userId || o.sellerId === req.userId);
...
router.get("/recommendations", auth, async (req, res) => {
    ...
    let recommended = approvedProducts.filter((p) => purchasedCategories.has(p.category) && !userPurchases.some((up) => up.productId === p.id));
    if (recommended.length < 3) {
        const remaining = approvedProducts.filter((p) => !recommended.includes(p) && !userPurchases.some((up) => up.productId === p.id));
