...
export function App() {
    ...
    const loadProfileStats = async () => {
        ...
        if (json.success) {
            setProfileStats(json.data);
        }
    };
    const rateOrder = async (order, rating) => {
        if (!rating || rating < 1 || rating > 10)
            return alert("请先选择 1 到 10 颗星再提交");
